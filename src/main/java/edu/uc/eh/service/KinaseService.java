...
//            geneName2KinaseNAmeJson = (JSONObject) parser.parse(geneName2KinaseNAmeString);
        ...
        for (int i = 0; i < input.length; i++) {
            ...
            if (kinase2GeneJson.containsKey(inputGene)) {
                log.info("kinase2GeneList");
                kinase2GeneList = (JSONArray) (kinase2GeneJson.get(inputGene));
                log.info(kinase2GeneList.toString());
            }

            if (gene2KinaseJson.containsKey(inputGene)) {
                log.info("gene2KinaseList");
                gene2KinaseList = (JSONArray) (gene2KinaseJson.get(inputGene));
